...
async def _active_or_latest(icp_collection):
...
async def get_icp_config(icp_collection=Depends(get_icp_collection)):
    try:
        return _serialize(await _active_or_latest(icp_collection))
...
async def add_industry(body: AddIndustryRequest, icp_collection=Depends(get_icp_collection)):
    if not body.displayName.strip():
        raise HTTPException(status_code=400, detail="displayName is required")
    doc = await _active_or_latest(icp_collection)
...
async def add_title(body: AddTitleRequest, icp_collection=Depends(get_icp_collection)):
...
async def add_location(body: AddLocationRequest, icp_collection=Depends(get_icp_collection)):
